/* SecureFileLocker.java
   Java Swing app with AES-GCM (256-bit) and PBKDF2.
   - Drag & drop with hover highlight
   - Temp-file-safe decryption (no partial decrypted file on wrong password)
   - Password wiping and automatic UI clearing after operations
...
public class SecureFileLocker {
    ...
    static abstract class PanelBase extends JPanel {
        ...
        void setStatus(String s) { if (statusBar!=null) statusBar.setStatus(s); }
        ...
        void enableAll(boolean v, Component... comps) {
            for (Component c: comps) if (c!=null) c.setEnabled(v);
        }

        /**
         * Install a DropTarget with hover highlight on the given JComponent.
         * Saves the original border/background/opaque state and restores them on exit/drop.
        ...
        void installDragDrop(JComponent comp) {
            ...
            DropTargetListener listener = new DropTargetListener() {
                ...
                @Override
                public void dragOver(DropTargetDragEvent dtde) { /* no-op */ }

                @Override
                public void dropActionChanged(DropTargetDragEvent dtde) { /* no-op */ }
    ...
    static class EncryptPanel extends PanelBase {
        ...
        EncryptPanel(StatusBar statusBar) {
            ...
            // Visual drop area
            JLabel dropLabel = new JLabel("<html><center>Drag &amp; drop file here<br/>or click Browse</center></html>", SwingConstants.CENTER);
            ...
            actions.add(clear); actions.add(enc);
            ...
            fileField.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
                private void changed() { updateFileInfo(); }
        ...
        private void updateFileInfo() {
            ...
            if (p.isEmpty()) { infoLabel.setText("No file selected"); return; }
            File f = new File(p);
            if (!f.exists()) { infoLabel.setText("Not found"); return; }
        ...
        private void startEncrypt() {
            ...
            if (path.isEmpty()) { JOptionPane.showMessageDialog(this, "Select a file to encrypt.", "Info", JOptionPane.INFORMATION_MESSAGE); return; }
            File in = new File(path);
            if (!in.exists()) { JOptionPane.showMessageDialog(this, "File not found.", "Error", JOptionPane.ERROR_MESSAGE); return; }
            char[] p1 = passField.getPassword();
            char[] p2 = confirm.getPassword();
            if (p1.length==0) { JOptionPane.showMessageDialog(this, "Password cannot be empty.", "Error", JOptionPane.ERROR_MESSAGE); return; }
            if (!Arrays.equals(p1,p2)) { JOptionPane.showMessageDialog(this, "Passwords do not match.", "Error", JOptionPane.ERROR_MESSAGE); wipe(p1); wipe(p2); return; }
            ...
            if (out.exists()) {
                ...
                if (c != JOptionPane.YES_OPTION) { wipe(p1); wipe(p2); return; }
            }
            ...
            SwingWorker<Void,Void> wk = new SwingWorker<>() {
                ...
                protected void done() {
                    try {
                        ...
                        setStatus("Error");
                        JOptionPane.showMessageDialog(EncryptPanel.this, "Encryption failed: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
                    } finally {
            ...
            wk.addPropertyChangeListener(evt -> { if ("progress".equals(evt.getPropertyName())) progressBar.setValue((Integer)evt.getNewValue()); });
            wk.execute();
        }
    }
    ...
    static class DecryptPanel extends PanelBase {
        DecryptPanel(StatusBar statusBar) {
            ...
            // Visual drop area
            JLabel dropLabel = new JLabel("<html><center>Drag &amp; drop .enc file here<br/>or click Browse</center></html>", SwingConstants.CENTER);
            dropLabel.setPreferredSize(new Dimension(260, 80));
            dropLabel.setHorizontalTextPosition(SwingConstants.CENTER);
            dropLabel.setVerticalTextPosition(SwingConstants.CENTER);
            dropLabel.setBorder(BorderFactory.createDashedBorder(Color.GRAY));
            ...
            actions.add(clear); actions.add(dec);
            bottom.add(actions, BorderLayout.EAST);
            bottom.add(progressBar, BorderLayout.CENTER);
        ...
        private void chooseFile() {
            ...
            int r = fc.showOpenDialog(this);
            if (r == JFileChooser.APPROVE_OPTION) fileField.setText(fc.getSelectedFile().getAbsolutePath());
        }
        ...
        private void clearAll() {
            fileField.setText(""); passField.setText(""); progressBar.setValue(0); setStatus("Ready");
        }

        private void startDecrypt() {
            String path = fileField.getText().trim();
            if (path.isEmpty()) { JOptionPane.showMessageDialog(this, "Select .enc file.", "Info", JOptionPane.INFORMATION_MESSAGE); return; }
            File in = new File(path);
            if (!in.exists()) { JOptionPane.showMessageDialog(this, "File not found.", "Error", JOptionPane.ERROR_MESSAGE); return; }
            char[] pw = passField.getPassword();
            if (pw.length==0) { JOptionPane.showMessageDialog(this, "Password cannot be empty.", "Error", JOptionPane.ERROR_MESSAGE); return; }
                ...
                protected void done() {
                    try {
                        ...
                        if (choice == JOptionPane.YES_OPTION) {
                            ...
                            JOptionPane.showMessageDialog(DecryptPanel.this, "Decryption completed: " + out.getAbsolutePath(), "Done", JOptionPane.INFORMATION_MESSAGE);
                        }
                        setStatus("Decryption complete");
                    } catch (Exception ex) {
                        setStatus("Error");
                        JOptionPane.showMessageDialog(DecryptPanel.this, "Decryption failed: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
                    } finally {
        ...
        private void openFile(File f) {
            if (!Desktop.isDesktopSupported()) {
                JOptionPane.showMessageDialog(this, "Opening files is not supported on this platform.", "Info", JOptionPane.INFORMATION_MESSAGE);
                return;
            }
            Desktop desk = Desktop.getDesktop();
            if (!desk.isSupported(Desktop.Action.OPEN)) {
                JOptionPane.showMessageDialog(this, "Open action not supported on this platform.", "Info", JOptionPane.INFORMATION_MESSAGE);
                return;
            }
            try {
                desk.open(f);
    ...
    static class CryptoService {
        ...
        interface Progress { void onProgress(long processed, long total); }
        ...
        static void encryptToFile(File input, File output, char[] password, Progress cb) throws Exception {
            ...
            try (FileInputStream fis = new FileInputStream(input);
                ...
                try (CipherOutputStream cos = new CipherOutputStream(bos, cipher)) {
                    ...
                    int r; long processed = 0;
                    while ((r = bis.read(buf)) != -1) {
                        cos.write(buf, 0, r);
                        processed += r;
                        if (cb!=null) cb.onProgress(processed, total);
                    }
                    cos.flush();
                }
            } finally {
        ...
        static class DecryptResult { final File output; DecryptResult(File f){this.output=f;} }

        /**
         * decryptToFile:
         * - reads header (salt/iv/original name),
         * - writes decrypted output to a temporary file in same directory,
         * - only when fully successful, moves temp to final "decrypted_<original>" filename,
         * - if any error occurs (AEAD failure/wrong password/tamper/IO), deletes temp and throws an exception.
         */
        static DecryptResult decryptToFile(File input, char[] password, Progress cb) throws Exception {
            ...
            try (FileInputStream fis = new FileInputStream(input);
                ...
                if (bis.read(magic)!=4) throw new IOException("Invalid file");
                for (int i=0;i<4;i++) if (magic[i]!=MAGIC[i]) throw new IOException("Not our encrypted file");
                int ver = bis.read();
                if (ver != VERSION) throw new IOException("Unsupported version");

                byte[] salt = new byte[SALT_LEN]; if (bis.read(salt)!=salt.length) throw new IOException("Invalid file (salt)");
                byte[] iv = new byte[IV_LEN]; if (bis.read(iv)!=iv.length) throw new IOException("Invalid file (iv)");
                byte[] lenB = new byte[8]; if (bis.read(lenB)!=8) throw new IOException("Invalid file (name len)");
                long nameLen = ByteBuffer.wrap(lenB).getLong();
                if (nameLen<0 || nameLen>4096) throw new IOException("Invalid filename length");
                byte[] nameB = new byte[(int)nameLen]; if (bis.read(nameB)!=nameB.length) throw new IOException("Invalid file (name)");
                ...
                try (FileOutputStream fos = new FileOutputStream(tmp);
                    ...
                    while ((r = cis.read(buf)) != -1) {
                        bos.write(buf, 0, r);
                        processed += r;
                        if (cb!=null) cb.onProgress(processed, totalCipher);
                    }
                    bos.flush();
        ...
        static SecretKey deriveKey(char[] password, byte[] salt) throws Exception {
            ...
            catch (Exception ex) { skf = SecretKeyFactory.getInstance("PBKDF2WithHmacSHA1"); }
